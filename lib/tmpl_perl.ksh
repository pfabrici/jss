...
#
#	- BESCHREIBUNG
#       Templatescript fuer die Ausfuehrung von Perl Dateien
...
#
#		CFGPERLFILE 	auszufuehrendes Perlskript
#
#       - LIZENZ
#
...
tmpl_perl()
{
	...
	while true ; do
		...
		OUTDATA=${OUTDATA:-/dev/null}
                ...
                #       gesucht
                #
                if [ -z ${CFGPERLFILE} ] ; then
                        [ -z {SQLDIR} ] && { ECODE=22; break; }
                        [ ! -d ${SQLDIR} ] && { ECODE=23; break; }

                        CFGPERLFILE=${SQLDIR}/${BASENAME}.pl
                        log runsqlfile "I: CFGPERLFILE=${CFGPERLFILE}"
                fi


                #       Statement ausfuehren
                #
                runperl ${CFGPERLFILE} ${INDATA} ${OUTDATA} || { ECODE=3; break; }
        ...
        log tmpl_perl "${MSG}"
        log tmpl_perl "Ende mit Exitcode ${ECODE}"
